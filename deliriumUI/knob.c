...
void displayDiliriumUIKnob(deliriumUI* deliriumUI_window, cairo_t* cr, int widgetNumber)
{
	...
	{
 		float cos_x =  (h/2.8) * (cos(((((1-scale)*0.75)-0.67)*2) * M_PI));
		float sin_y =  (h/2.8) * (sin(((((1-scale)*0.75)-0.67)*2) * M_PI));
 		float cos_x2 =  (h/2.4) * (cos(((((1-scale)*0.75)-0.67)*2) * M_PI));
		float sin_y2 =  (h/2.4) * (sin(((((1-scale)*0.75)-0.67)*2) * M_PI));
	...
	cairo_arc(cr, x + (w/2), y + (h/2.2), (h/3), 0.0, 2 * M_PI);
	cairo_fill(cr);
	cairo_set_source_rgb(cr, 0.0, 0.0, 0.0);
